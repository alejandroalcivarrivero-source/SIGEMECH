...
export const generarCodigoTemporal = (datos, provincias = []) => {
    ...
    const n1 = limpiarTexto(primerNombre).substring(0, 2).padEnd(2, 'A');
    const n2 = primerNombre && !segundoNombre ? 'A' : (limpiarTexto(segundoNombre).substring(0, 1) || 'A');

    // 2. Apellidos (3): 2 letras primer apellido + 1 letra segundo apellido (o '0').
    const a1 = limpiarTexto(primerApellido).substring(0, 2).padEnd(2, 'A');
    const a2 = primerApellido && !segundoApellido ? 'A' : (limpiarTexto(segundoApellido).substring(0, 1) || 'A');
    ...
    if (fechaNacimiento) {
        ...
        const f = new Date(fechaNacimiento.includes('T') ? fechaNacimiento : `${fechaNacimiento}T12:00:00Z`);
        if (!isNaN(f.getTime())) {
