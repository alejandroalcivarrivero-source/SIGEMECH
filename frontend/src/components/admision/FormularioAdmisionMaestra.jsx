...
import { NOMBRE_INSTITUCION_LOCAL } from '../../config/constants';
...
const FormularioAdmisionMaestra = () => {
    ...
    const initialFormData = {
        // Pestaña 1
        id_tipo_identificacion: '', numero_identificacion: '', primer_apellido: '', segundo_apellido: '',
        ...
        fecha_nacimiento: '', hora_nacimiento: '', id_autoidentificacion_etnica: '', id_nacionalidad_etnica: '', id_pueblo: '',
        
        // Pestaña 3
        pais_residencia_habitual: 'Ecuador', provinciaResidencia: '', cantonResidencia: '',
        id_parroquia: '', barrio: '', callePrincipal: '', calleSecundaria: '',
        numeroCasa: '', referencia_domicilio: '',
        
        // Pestaña 4
        id_nivel_instruccion: '', ocupacion: '', id_seguro_salud: '',
        tipo_empresa: '', tiene_discapacidad: false, tipo_discapacidad: '',
        porcentaje_discapacidad: '', carnet_discapacidad: '',
        
        // Pestaña 5
        nombres_tutor_legal: '', id_parentesco_representante: '',
        identificacion_tutor: '', direccion_representante: '',
        
        // Pestaña 6
        id_modalidad_arribo: '', id_fuente_informacion: '', persona_entrega: '', id_condicion_llegada: '',
        fecha_ingreso: new Date().toISOString().split('T')[0],
        establecimiento_origen: '',
        
        // Pestaña 7
        descripcion_motivo_atencion: '', motivo_detalle: '',
    ...
    const edadInfo = useMemo(() => {
        ...
        if (formData.hora_nacimiento) {
            birthDateStr += `T${formData.hora_nacimiento}`;
        } else {
        ...
        return {
            ...
            meses,
            dias,
            horas: formData.hora_nacimiento ? diffHours : 0,
            minutos: diffMinutes,
            isLess24h,
            isNeonato,
            esPartoReciente,
    ...
    const tabs = [
        { id: 'personales', label: '1. Filiación', icon: User },
        { id: 'nacimiento', label: '2. Registro de Nacimiento', icon: Baby },
        { id: 'residencia', label: '3. Residencia y Ubicación', icon: MapPin },
        { id: 'adicionales', label: '4. Datos Socio-Económicos', icon: ShieldCheck },
        { id: 'contacto', label: '5. Representante Legal', icon: Users },
        { id: 'llegada', label: '6. Modalidad de Arribo', icon: ClipboardList },
        { id: 'motivo', label: '7. Motivo de Atención', icon: HeartPulse }
    ];
    ...
    const handleChange = (e) => {
        ...
        if (name === 'id_tipo_identificacion') {
            ...
            if (esNoIdentificado) {
                ...
                resetData.descripcion_motivo_atencion = 'EMERGENCIA';
                ...
                resetData.numero_identificacion = codigoNormativo;
        ...
        setFormData(prev => {
            ...
            if (esNoIdentificado) {
                ...
                newData.numero_identificacion = nuevoCodigo;
            }
    ...
    const validarTemporada = (name, value) => {
        ...
        if (name === 'fecha_nacimiento') {
            ...
            if (formData.hora_nacimiento) {
                timestampValidacion = new Date(`${value}T${formData.hora_nacimiento}`);
            } else {
                return true;
            }
        } else if (name === 'hora_nacimiento') {
    ...
    const handleBlur = (e) => {
        ...
        if (name === 'fecha_nacimiento' || name === 'hora_nacimiento' || name === 'fecha_hora_parto') {
            if (value) {
    ...
    const handleBusquedaPaciente = async (value) => {
        ...
        if (!validarCedulaEcuatoriana(formData.numero_identificacion)) {
            setModalConfig({
        ...
        setLoading(true);
        try {
            const response = await pacienteService.findByCedula(formData.numero_identificacion);
            if (response.found) {
    ...
    const handleFinalize = async (e) => {
        ...
        const errores = validarAdmisionCompleta({
            ...formData,
            tipo_arribo: catalogos.formasLlegada.find(f => f.id === formData.id_modalidad_arribo)?.nombre
        });
        ...
        try {
            const payload = {
                pacienteData: {
                    ...
                    id_etnia: formData.id_autoidentificacion_etnica,
                    id_nacionalidad_etnica: formData.id_nacionalidad_etnica,
                    id_pueblo: formData.id_pueblo,
                    id_instruccion: formData.id_nivel_instruccion,
                ...
                admissionData: {
                    motivo_consulta: formData.descripcion_motivo_atencion,
                    motivo_detalle: formData.motivo_detalle,
                    id_forma_llegada: formData.id_modalidad_arribo,
                ...
                representanteData: {
                    nombre_representante: formData.nombres_tutor_legal,
                    id_parentesco_representante: formData.id_parentesco_representante,
                    documento_representante: formData.identificacion_tutor,
            ...
            // --- AUDITORÍA DE PAYLOAD (SMOKE TEST E2E FINAL) ---
            console.log("--------------------------------------------------");
            console.log("🚀 AUDITORÍA DE PAYLOAD - SIGEMECH SOBERANO");
            console.log("Payload Final:", JSON.stringify(payload, null, 2));
            
            // Verificación manual de soberanía lingüística antes del envío
            const llavesEnIngles = JSON.stringify(payload).match(/reasonForConsultation|documentNumber|firstName|lastName|birthDate/i);
            if (llavesEnIngles) {
                console.error("❌ ERROR DE SOBERANÍA: Se detectaron llaves en inglés en el payload:", llavesEnIngles);
            } else {
                console.log("✅ VERIFICACIÓN DE LENGUAJE: OK (100% Español Técnico)");
            }
            console.log("--------------------------------------------------");
            ...
            setModalConfig({
                ...
                confirmAction: () => {
                    if (formData.descripcion_motivo_atencion === 'EMERGENCIA' || formData.descripcion_motivo_atencion === 'TRIAGE' || formData.descripcion_motivo_atencion === '1') {
    ...
    const hasTemporalError = useMemo(() => {
        ...
        if (formData.fecha_nacimiento) {
            ...
            if (formData.hora_nacimiento) {
                const [hours, minutes] = formData.hora_nacimiento.split(':').map(Number);
                ...
                const esTipoCChone = estSeleccionado?.nombre?.toUpperCase().includes(NOMBRE_INSTITUCION_LOCAL);
        ...
        return false;
    }, [formData.fecha_nacimiento, formData.hora_nacimiento, catalogos.establecimientos, formData.id_lugar_parto]);
                ...
                <div className="bg-gradient-to-r from-blue-900 to-blue-700 p-4 text-white flex justify-between items-center border-b-4 border-yellow-400">
                    ...
                    <div className="bg-blue-800/50 border border-blue-400 rounded px-4 py-2 text-center animate-pulse">
                        ...
                        <p className="text-2xl font-black tracking-[0.2em] font-mono text-yellow-400">
                            {formData.numero_identificacion && (catalogos.tiposIdentificacion?.find(t => t.id == formData.id_tipo_identificacion)?.nombre?.toUpperCase() === 'NO IDENTIFICADO')
                    ...
                    <div className="text-right hidden md:block">
                        ...
                        <p className="text-sm font-black">{NOMBRE_INSTITUCION_LOCAL}</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleFinalize} className="p-4 md:p-5 space-y-4 bg-white min-h-[400px]">
                    ...
                    <div className="mt-12 flex flex-col md:flex-row justify-between items-center gap-4 border-t pt-8">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <button
                                type="submit"
                                disabled={loading || !formHabilitado || !formData.descripcion_motivo_atencion || hasTemporalError}
                                className={`group flex items-center px-12 py-4 font-black rounded-lg shadow-2xl transition-all transform ${
                                    loading || !formHabilitado || !formData.descripcion_motivo_atencion || hasTemporalError
