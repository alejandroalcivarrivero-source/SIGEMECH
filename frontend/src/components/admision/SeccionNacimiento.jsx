...
const SeccionNacimiento = ({ formData, handleChange, handleBlur, catalogos, formHabilitado, edadInfo, setFormData, setModalConfig, setNavegacionBloqueada }) => {
    ...
    const esMenorA24HorasReales = useMemo(() => {
        ...
        const hora = formData.hora_nacimiento || '00:00:00';
    ...
    useEffect(() => {
        const debeBloquear = esMenorA24HorasReales && !formData.hora_nacimiento;
        if (setNavegacionBloqueada) {
            setNavegacionBloqueada(debeBloquear);
        }
    }, [esMenorA24HorasReales, formData.hora_nacimiento, setNavegacionBloqueada]);


    const esRegistroValido = useMemo(() => {
        if (!formData.fecha_nacimiento) return false;
        if (esMenorA24HorasReales && !formData.hora_nacimiento) return false;
        
        if (formData.hora_nacimiento) {
            const timestampNacimiento = new Date(`${formData.fecha_nacimiento}T${formData.hora_nacimiento}`).getTime();
            if (isNaN(timestampNacimiento)) return false;
            ...
            if (!esMenorA24HorasReales) {
                ...
                if (est?.nombre?.toUpperCase().includes(NOMBRE_INSTITUCION_LOCAL)) return false;
            }
        }
        return true;
    }, [formData.fecha_nacimiento, formData.hora_nacimiento, esMenorA24HorasReales, formData.id_lugar_parto, catalogos.establecimientos]);
    ...
    useEffect(() => {
        const NOMBRE_RESTRINGIDO = NOMBRE_INSTITUCION_LOCAL;
        ...
        if (esMenorA24HorasReales && establecimientoChone) {
            // Inserta el establecimiento local al principio si no está ya.
            ...
            // Filtra el establecimiento local si la condición no se cumple.
    ...
    useEffect(() => {
        if (formData.fecha_nacimiento && formData.provinciaNacimiento) {
            ...
            if (tipoIdent?.nombre?.toUpperCase() === 'NO IDENTIFICADO' || !formData.numero_identificacion) {
                const nuevoCodigo = generarCodigoTemporal(formData, catalogos.provincias);
                if (nuevoCodigo !== formData.numero_identificacion) {
                    handleChange({
                        target: {
    ...
    useEffect(() => {
        const ESTABLECIMIENTO_LOCAL_NOMBRE = NOMBRE_INSTITUCION_LOCAL;
        ...
        try {
            ...
            if (data) {
                ...
                const updates = {
                    ...
                    identificacion_tutor: data.numero_documento,
                    nombres_tutor_legal: `${data.primer_nombre} ${data.segundo_nombre || ''} ${data.primer_apellido} ${data.segundo_apellido || ''}`.trim(),
                        ...
                        {esHoyOAnteriores24h && (
                            <div className="col-span-1 animate-in fade-in zoom-in duration-300">
                                ...
                                <input
                                            ...
                                            setContadorErroresHora(prev => {
                                                ...
                                                setModalConfig({
                                                    ...
                                                    confirmAction: nuevoContador >= 2 ? () => {
                                                        setFormData(prevData => ({ ...prevData, fecha_nacimiento: '', hora_nacimiento: '' }));
                                                        setContadorErroresHora(0);
                                                        setModalConfig(m => ({ ...m, show: false }));
                                                    } : null,
                                                    onClose: () => {
                                                        setFormData(prevData => ({ ...prevData, hora_nacimiento: '' }));
                                                        setModalConfig(m => ({ ...m, show: false }));
                                    ...
                                    required={true}
                                    className={`${inputClasses} font-bold text-blue-800 bg-white border-blue-400 ${!formData.hora_nacimiento ? 'ring-2 ring-red-200' : ''}`}
                                />
                            </div>
                        )}
                        ...
                        <div className="col-span-2">
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    onChange={handleChange}
                                    className={`${inputClasses} bg-white ${establecimientosSalud.length === 0 ? 'animate-pulse' : ''} ${(esMenorA24HorasReales && !formData.hora_nacimiento) ? 'opacity-50 cursor-not-allowed grayscale' : ''}`}
                                    disabled={!formHabilitado || establecimientosSalud.length === 0 || (esMenorA24HorasReales && !formData.hora_nacimiento)}
                                >
                                    <option value="">{(esMenorA24HorasReales && !formData.hora_nacimiento) ? '--- INGRESE HORA PRIMERO ---' : 'Seleccione Establecimiento...'}</option>
