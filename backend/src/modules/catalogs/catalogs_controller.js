...
// Los modelos se obtienen dinámicamente para evitar ReferenceError durante la inicialización
...
async function getNivelesEducacion(req, res) {
    const { NivelInstruccion } = getModels();
...
async function getFormasLlegada(req, res) {
    const { FormularioLlegada } = getModels();
...
async function getFuentesInformacion(req, res) {
    const { FuenteInformacion } = getModels();
...
async function getCondicionesLlegada(req, res) {
    const { CondicionLlegada } = getModels();
...
async function getTiposIdentificacion(req, res) {
    const { TipoIdentificacion } = getModels();
...
async function getEthnicNationalities(req, res) {
    const { AutoidentificacionEtnica } = getModels();
...
async function getEstablecimientosSalud(req, res) {
    const { EstablecimientoSalud } = getModels();
